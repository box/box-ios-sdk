            ...
            describe("init()") {
                ...
                it("should correctly deserialize a file type from full JSON representation") {
                    ...
                    do {
                        ...
                        let folderItem = try FolderItem(json: jsonDict)

                        guard case let .file(file) = folderItem else {
                ...
                it("should correctly deserialize a folder type from full JSON representation") {
                    ...
                    do {
                        ...
                        guard case let .folder(folder) = folderItem else {
                ...
                it("should correctly deserialize a webLink type from full JSON representation") {
                    ...
                    do {
                        ...
                        guard case let .webLink(webLink) = folderItem else {
                ...
                it("should throw BoxCodingError.valueMismatch exception when deserialize an object with an unknown value in type filed") {
                    guard let filepath = Bundle(for: type(of: self)).path(forResource: "FullFolderItem_ValueFormatMismatch", ofType: "json") else {
                        fail("Could not find fixture file.")
                        return
                    }
                    ...
                    do {
                        let contents = try String(contentsOfFile: filepath)
                        let folderItemDictionary = try JSONSerialization.jsonObject(with: contents.data(using: .utf8)!)
                        ...
                        expect(try FolderItem(json: folderItemDictionary as! [String: Any])).to(throwError(expectedError))
                    }
                    catch {
                        fail("Failed with Error: \(error)")
                    }
                ...
                it("should throw BoxCodingError.typeMismatch exception when deserialize object with no type field") {
                    guard let filepath = Bundle(for: type(of: self)).path(forResource: "FullFolderItem_MissingRequiredField", ofType: "json") else {
                        fail("Could not find fixture file.")
                        return
                    }
            ...
            describe("rawData") {
                it("should be equal to json data used to create the FolderItem file type object") {
                    ...
                    do {
                        ...
                        expect(JSONComparer.match(json1: folderItem.rawData, json2: jsonDict)).to(equal(true))
                        expect(folderItem.json()).to(equal(folderItem.toJSONString()))
                    }
                    catch {
                        fail("Failed with Error: \(error)")
                    }
            ...
            describe("debugDescription") {
                it("should return correct description for a file type") {
                    ...
                    do {
                        ...
                        let jsonDict = try JSONSerialization.jsonObject(with: contents.data(using: .utf8)!) as! [String: Any]
                        let folderItem = try FolderItem(json: jsonDict)
                        expect(folderItem.debugDescription).to(equal("file Optional(\"script.js\")"))
                    }
                    catch {
                        fail("Failed with Error: \(error)")
                    }
                ...
                it("should return correct description for folder type") {
                    ...
                    do {
                        ...
                        expect(folderItem.debugDescription).to(equal("folder Optional(\"Test Folder\")"))
                    }
                    catch {
                        fail("Failed with Error: \(error)")
                    }
                ...
                it("should return correct description for webLink type") {
                    ...
                    do {
                        ...
                        expect(folderItem.debugDescription).to(equal("web link Optional(\"Example Web Link\")"))
                    }
                    catch {
                        fail("Failed with Error: \(error)")
                    }
