...
class MetadataTaxonomiesManagerTests: RetryableTestCase {
    ...
    public func testMetadataTaxonomiesNodes() async throws {
        await runWithRetryAsync {
            ...
            let fieldDisplayName: String = "testTaxonomy123"
            let fieldKey: String = "testTaxonomy321"
            let metadataTemplate: MetadataTemplate = try await client.metadataTemplates.createMetadataTemplate(requestBody: CreateMetadataTemplateRequestBody(scope: "enterprise", displayName: metadataTemplateKey, templateKey: metadataTemplateKey, fields: [CreateMetadataTemplateRequestBodyFieldsField(type: CreateMetadataTemplateRequestBodyFieldsTypeField.taxonomy, key: fieldKey, displayName: fieldDisplayName, taxonomyKey: taxonomyKey, namespace: namespace, optionsRules: CreateMetadataTemplateRequestBodyFieldsOptionsRulesField(multiSelect: true, selectableLevels: [Int64(1)]))]))
            ...
            XCTAssertTrue(metadataTemplate.fields![0].displayName == fieldDisplayName)
            XCTAssertTrue(metadataTemplate.fields![0].hidden == false)
            XCTAssertTrue(metadataTemplate.fields![0].id != "")
            XCTAssertTrue(metadataTemplate.fields![0].key == fieldKey)
            XCTAssertTrue(metadataTemplate.fields![0].namespace == namespace)
            XCTAssertTrue(metadataTemplate.fields![0].optionsRules!.multiSelect == true)
            XCTAssertTrue(metadataTemplate.fields![0].optionsRules!.selectableLevels![0] == 1)
            XCTAssertTrue(metadataTemplate.fields![0].taxonomyId != "")
            XCTAssertTrue(metadataTemplate.fields![0].taxonomyKey == taxonomyKey)
            XCTAssertTrue(Utils.Strings.toString(value: metadataTemplate.fields![0].type) == "taxonomy")
            let options: MetadataTaxonomyNodes = try await client.metadataTaxonomies.getMetadataTemplateFieldOptions(namespace: namespace, templateKey: metadataTemplateKey, fieldKey: fieldKey)
