...
class AiManagerTests: RetryableTestCase {
    ...
    public func testAiExtract() async throws {
        await runWithRetryAsync {
            let aiAgentConfig: AiAgent = try await client.ai.getAiAgentDefaultConfig(queryParams: GetAiAgentDefaultConfigQueryParams(mode: GetAiAgentDefaultConfigQueryParamsModeField.extract, language: "en-US"))
    ...
    public func testAiExtractStructuredWithFields() async throws {
        await runWithRetryAsync {
            ...
            XCTAssertTrue(Utils.Strings.toString(value: response.answer["dateOfBirth"]) == "1990-07-04")
            XCTAssertTrue(Utils.Strings.toString(value: response.answer["age"]) == "34")
            XCTAssertTrue(Utils.Strings.toString(value: response.answer["address"])!.contains("Redwood City"))
            XCTAssertTrue(Utils.Strings.toString(value: response.answer["address"])!.contains("CA"))
            XCTAssertTrue(Utils.Strings.toString(value: response.answer["address"])!.contains("94063"))
            XCTAssertTrue(Utils.Strings.toString(value: response.answer["work_history"])!.contains("Box"))
            XCTAssertTrue(response.completionReason == "done")
            try await client.files.deleteFileById(fileId: file.id)
        }
    }
    ...
    public func testAiExtractStructuredWithMetadataTemplate() async throws {
        await runWithRetryAsync {
            ...
            XCTAssertTrue(Utils.Strings.toString(value: response.answer["firstName"]) == "John")
            XCTAssertTrue(Utils.Strings.toString(value: response.answer["lastName"]) == "Doe")
            XCTAssertTrue(Utils.Strings.toString(value: response.answer["dateOfBirth"]) == "1990-07-04T00:00:00Z")
            XCTAssertTrue(Utils.Strings.toString(value: response.answer["age"]) == "34")
            XCTAssertTrue(Utils.Strings.toString(value: response.answer["hobby"]) == Utils.Strings.toString(value: ["guitar"]))
            XCTAssertTrue(response.completionReason == "done")
