...
public class BoxNetworkClient: NetworkClient {
    ...
    public func withTimeoutConfig(timeoutConfig: TimeoutConfig) -> BoxNetworkClient {
        let baseConfig = (self.session.configuration.copy() as? URLSessionConfiguration) ?? self.session.configuration
        return BoxNetworkClient(configuration: baseConfig, timeoutConfig: timeoutConfig)
    }
